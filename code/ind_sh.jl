...
function ind_sh(expmval, expmu, res::Results, data::Data)
    @unpack ns, cdindex, cdid = data #I am unsure exactly where ns, cdindex, and cdid are defined. My guess is that cdindex may mean census data index and cdid may refer to census data id? A bit lost on what ns means here, maybe number of samples (i.e. 20 samples in each location)? 
    ...
    sum1[2:length(sum1[:,1], :] = diff(sum1) #not entirely sure my syntax is right here
